...
UsersCommon::removeUser( QListWidgetItem* currentItem )
{
    ...
    // Remove user
    QString asUserFileName = QString( "/var/lib/AccountsService/users/%1" ).arg( username );
    if ( QFile::exists( asUserFileName ) )
    {
        QFile asUserFile (asUserFileName);
        asUserFile.remove();
    }
...
UsersCommon::setUserImage( Ui::PageUsers* ui )
{
    ...
    if ( QDir( "/var/lib/AccountsService/icons/" ).exists() )
    {
        ...
        QString iconLine = QString( "Icon=/var/lib/AccountsService/icons/%1" ).arg( item->user.username );
        QString asUserFileName = QString( "/var/lib/AccountsService/users/%1" ).arg( item->user.username );

        QFile asUserFile(asUserFileName);

        if ( QFile::exists( asUserFileName ) )
        {
            if (! asUserFile.open (QIODevice :: Append | QIODevice::Text))
            {
                qDebug() << "Can't open file";
            }
            else
            {
                QTextStream stream (& asUserFile);
                stream << iconLine;
                asUserFile.close ();
            }
        }
        ...
        {
            asUserFile.open(QIODevice::WriteOnly);
            QTextStream out(&asUserFile);
            out << "[User]" << endl;
            out << "XSession=xfce" << endl;
            out << "SystemAccount=false" << endl;
            out << iconLine << endl;
